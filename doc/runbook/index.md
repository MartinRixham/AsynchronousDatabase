...
| `503` rather than `200` | `unled` or `draining` is `true`. Every field is still there to read; the status is for the load balancer, which reads nothing else |
...
| `incomplete` | `true` is a node holding less than it owns — [a rebuild that did not read the whole of its share](/runbook/rebuild#a-node-that-came-up-short), or a partition a membership change handed it that it has not fetched yet. It still serves what it has, and answers `node_incomplete` rather than calling a key missing |
| `unled` | `true` is a node that has been unable to order a write for longer than a lease — etcd has stopped answering it, or names no node but it — and so refuses every write with `no_leader`: [etcd cannot be reached](/runbook/membership#etcd-cannot-be-reached), usually. It is the one field the status carries, because a node taking no writes is one the load balancer should stop choosing and nothing else tells it so. Never `true` on an instance that was [never clustered](/database/reference#the-cluster), which takes the writes it is given |
| `draining` | `true` is a node that was told to stop and is serving out its [`ASYNCDB_DRAIN`](/database/reference#the-cluster) seconds, so that the load balancer takes it out before it stops answering. It is never `true` for long: the process closes when the drain is over |
| `nodes` | The membership **as this node sees it**. A list naming only this node is [a node alone](/runbook/membership#a-node-alone), which refuses a read with `node_alone`; a node that has lost etcd keeps the list it last read, and says so in `etcd`. Absent entirely is an instance that was never clustered at all |
