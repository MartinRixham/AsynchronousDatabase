...
test.describe("create table", () => {
	...
	test("adds a table that depends on another and sees it in the graph",
		async ({ page, database }) => {
			...
			// The form closes and the new table joins the graph, in the row under the one it
			// depends on and with an arrow of its own.
	...
	test("shows the error a name that is taken is answered with", async ({ page, database }) => {
		...
		expect(await database.tables()).toHaveLength(2);
	});
});
