...
- **Validation failures are values, not exceptions.** A table is `std::expected<table::table,
  ...
  refuses to persist one. `record::parse_record` and `parse_key` answer a record the same way, and
  `scan::parse_range` a range. Follow this pattern rather than throwing; exceptions are reserved
  for genuine infrastructure failure — `repository::storage_error` carries `storage_error` or
  `write_stalled`, and `located(...)` from `src/error.h` prefixes the file, line and function it
  was called from.
...
- **A leader is claimed in etcd, not elected by votes, and which node claims is decided by the
  ...
  kept in the store, written in one batch with the record that raised it or ahead of the file that
  carried it (`repository::read_terms`), and the router hands it to `cluster::restore_terms` as it is
  constructed — the tables' term taken from the schema's stamps — as a rebuild or a reconcile pass
  does after every file it takes in, so a record that moved is fenced where it lands and a restart onto a kept volume
